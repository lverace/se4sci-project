...
class Decay(ThreeDScene):
    def construct(self):
        ...
        parent_radius = 0.5
        parent = Sphere(radius=parent_radius)
        parent.set_color(BLUE)

        # Construct vectors, no positioning yet
        scaling_factor = 1.4

        K_vec = scaling_factor * np.array([df["K_px"][decay_num], df["K_py"][decay_num], df["K_pz"][decay_num]])
        pi_m2_vec = scaling_factor * np.array([df["pi_minus_2_px"][decay_num], df["pi_minus_2_py"][decay_num], df["pi_minus_2_pz"][decay_num]])
        pi_m3_vec = scaling_factor * np.array([df["pi_minus_3_px"][decay_num], df["pi_minus_3_py"][decay_num], df["pi_minus_3_pz"][decay_num]])
        pi_p4_vec = scaling_factor * np.array([df["pi_plus_4_px"][decay_num], df["pi_plus_4_py"][decay_num], df["pi_plus_4_pz"][decay_num]])
        ...
        daughter1_vec = Arrow3D(
            start=daughter1_start,
            end=daughter1_end,
            resolution=8
        )
        ...
        daughter2_vec = Arrow3D(
            start=daughter2_start,
            end=daughter2_end,
            resolution=8
        )
        ...
        daughter1_sphere = Sphere(center=daughter1_sphere_center, radius=daughter1_sphere_r)
        daughter1_sphere.set_color(RED)
        ...
        daughter2_sphere = Sphere(center=daughter2_sphere_center, radius=daughter2_sphere_r)
        daughter2_sphere.set_color(PURPLE)
        ...
        K_arrow = Arrow3D(
            start=k_pi2_start,
            end=[k_pi2_start[0] + K_vec[0], k_pi2_start[1] + K_vec[1], k_pi2_start[2] + K_vec[2]],
            resolution=8
        )
        ...
        pi2_arrow = Arrow3D(
            start=k_pi2_start,
            end=[k_pi2_start[0] + pi_m2_vec[0], k_pi2_start[1] + pi_m2_vec[1], k_pi2_start[2] + pi_m2_vec[2]],
            resolution=8
        )
        ...
        pi3_arrow = Arrow3D(
            start=pi3_pi4_start,
            end=[pi3_pi4_start[0] + pi_m3_vec[0], pi3_pi4_start[1] + pi_m3_vec[1], 
        ...
        pi4_arrow = Arrow3D(
            start=pi3_pi4_start,
            end=[pi3_pi4_start[0] + pi_p4_vec[0], pi3_pi4_start[1] + pi_p4_vec[1], 
            pi3_pi4_start[2] + pi_p4_vec[2]],
            resolution=8
        )

        # Set arrow colors
        K_arrow.set_color(RED)
        pi2_arrow.set_color(ORANGE)
        pi3_arrow.set_color(BLUE)
        pi4_arrow.set_color(PURPLE)
        ...
        if (animation_mode == "picture"):
            ...
            self.set_camera_orientation(phi=90 * DEGREES, theta=270 * DEGREES) # phi=90, theta=270 shows side profile with +x axis to the right
            ...
            self.wait(2)
            daughter_group = VGroup(daughter1_vec, daughter2_vec)
            self.play(Create(daughter_group))

            dsphere_group = VGroup(daughter1_sphere, daughter2_sphere)
            self.play(Create(dsphere_group))
            self.wait(2)

            self.play(Create(K_arrow), Create(pi2_arrow), Create(pi3_arrow), Create(pi4_arrow))
            self.wait(2)
